...
public class RoleAction extends BasicAction{
    ...
    public Map<String, Object> addrole(RoleEntity roleEntity,String right,
		...
		try {
			...
			if(!Validator.isFullName(roelname)){
				errMsg = "角色名称不可含有除点以外的特殊字符且长度不可超过20位";
    			log.info(errMsg);
    			object.put("result", -1);
    			object.put("message", errMsg);
				return object;
    ...
    public Map<String, Object> update(RoleEntity roleEntity,String right,
		...
		try {
			...
			object.put("result", 1);
			log.info(loaduser.getUserName()+"修改角色成功>>>>>>>>>>>>>>>>");
		} catch (BusinessException e) {
            e.printStackTrace();
            log.error("修改角色信息异常，具体异常信息："+ e.getMessage()+">>>>>>>>>>>>");
            object.put("result", -1);
            object.put("message", "修改角色信息异常");
        } catch (Exception e) {
			e.printStackTrace();
			log.error("修改角色信息异常，具体异常信息："+ e.getMessage()+">>>>>>>>>>>>");
			object.put("result", -1);
    ...
    public Map<String, Object> toupdaterole(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			object.put("result", -1);
			object.put("message", "修改角色前查询信息异常");
		}
