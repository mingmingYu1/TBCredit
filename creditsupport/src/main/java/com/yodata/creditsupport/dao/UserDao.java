...
@Component
public class UserDao extends SimpleHibernateDao<UserEntity, java.lang.Integer>{
    ...
    public Page<User> getPageUsers(String condition, String values, Page<User> page) throws DataAccessException {
        ...
        try {
            ...
            String countSql = dynamicHibernateSqlDao.getNamedQuerySql(
                    constant.getSqlGetPageUsers() + constant.getSqlCount(), params);
            ...
            String dataSql = dynamicHibernateSqlDao.getNamedQuerySql(
                    constant.getSqlGetPageUsers(), params);
    ...
    public UserEntity findUserEntity(UserEntity user) {
        Criteria criteria = this.createCriteria(Restrictions.eq("userName", user.getUserName()));
    ...
    public List<Integer> queryRoleByUserId(Integer uid) throws DataAccessException {
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("userId", uid);
        String sql = "";
        try {
            sql = dynamicHibernateSqlDao.getNamedQuerySql(
                    constant.getSqlQueryRoleByUserId(), params);
        } catch (DataAccessException e) {
            e.printStackTrace();
            log.error(e.getMessage(), e);
            throw e;
