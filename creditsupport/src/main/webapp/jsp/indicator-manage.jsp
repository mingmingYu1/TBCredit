...
<div class="row" id="indicatorManage">
    <div class="col-xs-12 overflow nonePaddingLeft">
        ...
        <div class="oneHeader">
            <p class="sumTitle"><a href="./score-manage.jsp">评分模型</a> > 指标体系</p>
        </div>
    </div>
    <div class="col-xs-12 marginLeft">
        <div class="searchInput overflow">
            <div class="pull-left">
                ...
                <input type="text" class="form-control" id="indicatorCode_condition" placeholder="请输入指标编号">
            </div>
            <div class="pull-left" style="margin-left: 200px;">
                <label for="indicatorName_condition">指标名称：</label>
                <input type="text" class="form-control" id="indicatorName_condition" placeholder="请输入指标名称">
            </div>
...
<!--                     </thead> -->
<!--                 </table> -->
                <table id="indicator_table" class="table table-striped"></table>
            </div>
        </div>
    </div>
</div>
...
<!-- 新增、修改页面 -->
<div class="modal fade" data-backdrop="static" id="indicatorModal" tabindex="-1" role="dialog"  aria-labelledby="indicatorModal" aria-hidden="true">
                ...
                <form class="form-horizontal" role="form" id="indicator_form"   action="/creditsupport/indicator/update" method="post">
                    ...
                    <div class="modalOne overflow">
                        <div>
                            ...
                            <input class="form-control" type="text" id="indicatorCode_manage" 
                                 maxlength="20" name="indCode" placeholder="数字或字母或“_”且长度不超过20位" title="数字或字母或“_”且长度不超过20位" />
                        </div>
                        <div>
                            <label for="indicatorName_manage"><span>*</span>指标名称：</label>
                            <input class="form-control" type="text" id="indicatorName_manage" maxlength="20" name="indName" 
                                 placeholder="不可含有除点以外的特殊字符且长度不可超过20位" title="不可含有除点以外的特殊字符且长度不可超过20位" />
                        </div>
                    </div>
                    ...
                    <div class="modalThree overflow">
                        ...
                        <div>
                            ...
                            <input class="form-control" id="unit_manage" name="unit" placeholder="长度不可超过16位" maxlength="16" />
                        </div>
                    </div>
                    <div class="modalFour overflow">
                        ...
                        <div>
                            <textarea class="form-control" id="indicatorDesc_manage" name="indDesc" placeholder="长度不可超过100位" maxlength="100"></textarea>
                        </div>
                    </div>
                    <div class="modalFive overflow">
                        ...
                        <div>
                            <textarea class="form-control" id="calculRule_manage" name="calaRegu" maxlength="100" placeholder="长度不可超过100位，例如：（本月销量-上月销量）/本月销量*100"></textarea>
                        </div>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    var columns = [
        ...
        },
        {
            ...
            title: '操作',
            align: 'right',
            formatter: operateFormatterIndicator,
            events: 'operateEventsIndicator'
        }
    ]
    $('#indicator_table').bootstrapTable('destroy').bootstrapTable({
        columns: columns,
        cache: false,
        pagination: true,
    ...
    function operateFormatterIndicator(value, row, index) {
        return  '<mrp:p rId="312"><a href="javascript:void(0)"  class="indicator_remove _remove" title="删除">删除</a></mrp:p>';
    }
    ...
    function search_indicator() {
        var indicatorCode = encodeURI($("#indicatorCode_condition").val());
        var indicatorName = encodeURI($("#indicatorName_condition").val());
        var level1 = encodeURI($("#indicatorType_condition").val());
//      var params = {
//          indicatorCode : indicatorCode,
//          indicatorName : indicatorName,
//          level1 : level1
//      };
        $('#indicator_table').bootstrapTable('refresh', {
            url: "/creditsupport/indicator/query",
            indicatorCode : indicatorCode,
            indicatorName : indicatorName,
            level1 : level1
...
//      });
  ...
  function paginationParam(params) {
    var indCode = encodeURI($("#indicatorCode_condition").val());
    var indName = encodeURI($("#indicatorName_condition").val());
    var level1 = encodeURI($("#indicatorType_condition").val());
    return {
      ...
      offset: params.offset,
            pageNumber: $('#indicator_table').bootstrapTable('getOptions').pageNumber
    };
  }
    ...
    $("#indicatorCode_condition").on("change", function(e) {
        ...
        if (value !== "") {
            ...
            isValidate = checkData(isValidate, value, "指标编号", checkLength, 20);
            if(!isValidate){
        ...
        callback : {
            ...
            onClick : function(event, treeId, treeNode) {
                if(treeNode.level >= 6 && treeNode.nocheck){
                    window.wxc.xcConfirm("不能选择第七级指标为上级指标", window.wxc.xcConfirm.typeEnum.info);
                    return false;
                }
                if(treeNode.nocheck){
                    window.wxc.xcConfirm("不能选择指标本身为上级指标",  window.wxc.xcConfirm.typeEnum.info);
                    return false;
                }
    ...
    function indicatorCheck() {
        //指标编号非空、非汉字及字段长度校验
        var indicatorCode = $("#indicatorCode_manage").val();
        if (indicatorCode === "") {
            window.wxc.xcConfirm("指标编号不可为空!", window.wxc.xcConfirm.typeEnum.info);
            return false;
        }
        if(!checkNumber(indicatorCode, '指标编号')) {
            return false;
        }
        ...
        if(!checkNameString(indicatorName, '指标名称')) {
            return false
        }
        if (indicatorName && indicatorName.length > 20) {
            window.wxc.xcConfirm("指标名称长度不可超过20个字符", window.wxc.xcConfirm.typeEnum.info);
            return false;
        }
        //上级指标
        ...
        if (unit && unit.length > 16) {
            window.wxc.xcConfirm("指标单位长度不可超过16位",
        ...
        if (indicatorDesc && indicatorDesc.length > 100) {
            window.wxc.xcConfirm("指标描述长度不可超过100位",
                    window.wxc.xcConfirm.typeEnum.info);
            return false;
        }
        ...
        if (calculRule && calculRule.length > 100) {
            window.wxc.xcConfirm("计算规则长度不可超过100个字符",    window.wxc.xcConfirm.typeEnum.info);
            return false;
        }
    }
