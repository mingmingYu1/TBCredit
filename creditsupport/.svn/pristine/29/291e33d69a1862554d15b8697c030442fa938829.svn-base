$(document).ready(function () {
	...
	$.ajax({
		url: '/creditsupport/ticketDetal/retirement',
		data: {
			id: oddId
		},
		dataType: 'json',
		success: function (data){
			if(data.code == 1 ){
				...
				console.log(arr)
				setChart('#retirementTicketChart',data.mOutList,data.mBackList,data.mPercentList,arr);
			}
		}
	});
	...
	function setChart(o,mOutList,mBackList,mPercentList,array){
		var dom = o.replace(/#/,"");
			...
			yAxis: [
				{
					...
					gridLineWidth: 0,
					opposite:true,
					min: getMInMax(mPercentList).min,
					max: getMInMax(mPercentList).max,
					labels : {
						formatter : function () {
